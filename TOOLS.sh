...
send_to_whatsapp_rapid() {
    ...
    while true; do
        ...
        if [ $current_threads -lt $max_threads ]; then
            ...
            local waul="$(uuidgen | tr -d '-')"
            local rand_email="hozooimut${RANDOM}${RANDOM}@gmail.com"
            local post_data="Nomor Telepon=${phone}&Alamat Email=${rand_email}&Konfirmasikan Alamat Email=${rand_email}&Bagaimana Anda menggunakan WhatsApp=Android&Silakan masukkan pesan Anda di bawah=Saya menulis untuk meminta bantuan terkait akun WhatsApp saya yang diblokir. Nomor telepon saya yang terdaftar di WhatsApp adalah ${phone}. Saya tidak melakukan tindakan yang melanggar ketentuan layanan WhatsApp, namun akun saya tetap diblokir tanpa penjelasan yang jelas.mohon bantuannya supaya nomor saya dibuka kembali karena ada grup sekolah yang setiap harinya ada guru yang mengirimkan tugas melalui WhatsApp jika nomor saya tidak di buka kembali dari banned bagaimana cara saya mengetahui jika ada tugas yang dikirim melalui grup sekolah sekali lagi mohon bantuannya terimakasih &country=ID&phone_code=62&useTrustedTypes=false&isTrustedTypesReportOnly=false&ajaxpipe_token=${ajaxpipe_token}&compat_iframe_token=${compat_iframe_token}&isCQuick=false&brsid=7525697063073108760&waul=${waul}"
            local user_agents=(
                "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/$(shuf -i 100-120 -n 1).0.0.0 Safari/537.36"
                "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_$(shuf -i 12-16 -n 1)_$(shuf -i 0-9 -n 1)) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/$(shuf -i 10-16 -n 1).$(shuf -i 0-5 -n 1) Safari/605.1.15"
                "Mozilla/5.0 (X11; Linux x86_64; rv:$(shuf -i 100-120 -n 1).0) Gecko/20100101 Firefox/$(shuf -i 100-120 -n 1).0"
